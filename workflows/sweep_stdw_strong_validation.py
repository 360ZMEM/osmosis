"""Small STDW strong-validation matrix driver.

Runs the approved 2026-06-13 small matrix for COM-COB step/gradual injection,
single-thruster efficiency loss, water-density scale, and runtime torque pulses.
"""
...
def build_cases() -> list[dict[str, Any]]:
    ...
    stdw_modes = [True, False]
    for embodiment in embodiments:
        for use_stdw in stdw_modes:
            for ramp_shape in ["linear", "step"]:
                cases.append({
                    "name": f"cob_{ramp_shape}_{embodiment}_stdw{int(use_stdw)}",
                    "group": "cob",
                ...
                fault_profile = "ramp_to_target" if ramp_shape == "linear" else "fixed"
                cases.append({
                    "name": f"thruster_single_{fault_profile}_{embodiment}_stdw{int(use_stdw)}",
                ...
                cases.append({
                    "name": f"density095_{ramp_shape}_{embodiment}_stdw{int(use_stdw)}",
                    "group": "density",
            ...
            for level, scenario in [(0.5, "torque_pulse_medium"), (1.0, "torque_pulse_strong")]:
                cases.append({
                    "name": f"torque_l{str(level).replace('.', 'p')}_{embodiment}_stdw{int(use_stdw)}",
                    "group": "torque_pulse",
                    "scenario": scenario,
                    "ramp_shape": "linear",
                    "embodiment": embodiment,
                    "use_stdw": use_stdw,
                    "extra": ["--torque_pulse_level", str(level)],
                })
    return cases
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run small STDW strong-validation matrix.")
    parser.add_argument("--profile", default="small", choices=["small"])
    parser.add_argument("--results_root", default=".results/exp_stdw_strong_validation_20260613")
    ...
    for idx, case in enumerate(cases, 1):
        ...
        row = {
            ...
            "mean_roll_mse": summary.get("mean_roll_mse"),
            "mean_pitch_mse": summary.get("mean_pitch_mse"),
            "mean_yaw_mse": summary.get("mean_yaw_mse"),
    ...
    out = root / "strong_validation_runs.csv"
